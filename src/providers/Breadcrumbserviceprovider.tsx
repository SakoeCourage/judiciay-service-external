...
export const Breadcrumbserviceprovider = ({ children }: { children: React.ReactNode }) => {
    ...
    return <BreadcrumbContext.Provider value={value}>
        {showBreadcrumbs && <nav className="flex container mx-auto px-10 pt-4 pb-3 max-w-[100dwh] overflow-scroll hiddenscroll" aria-label="Breadcrumb">
            <ol className="inline-flex items-center space-x-1 rtl:space-x-reverse">
                {
                    !!breadcrumbs.length && breadcrumbs.map((breadcrumb, i) => {
                        return <li key={i}>
                            <Link href={breadcrumb.path} className="inline-flex hover:text-blue-500 hover:underline whitespace-nowrap items-center text-sm font-normal text-gray-700 ">
